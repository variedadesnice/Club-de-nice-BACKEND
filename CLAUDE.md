...
> ⚠️ `analytics_daily_snapshots` does **not** follow these rules. Its rows are written by the Postgres function `generate_daily_snapshot()` (called by the `daily-analytics-snapshot` pg_cron job and by `POST /api/admin/analytics/snapshot`), which lives in Supabase and counts on its own criteria — `invited_members` there also compares role case-sensitively, so capitalized `"Invitado"` rows were missed. The history chart in the admin panel reads those rows, so it's on older criteria than the live figures. A corrected version of that function, following the same rules as the Python side, is checked in at `sql/generate_daily_snapshot.sql` — it has to be applied by hand in the Supabase SQL editor, and its header explains what to compare before running it. Rows written before you apply it keep the old criteria.
