...
SENDHOOK_WEBHOOK_SECRET=whsec_...               # Optional — HMAC secret for POST /api/webhooks/sendhook. Unset = webhook answers 503, only the backstop retries run.
```
...
**Automatic Pago Móvil verification (SendHook)**: if `SENDHOOK_API_URL`/`SENDHOOK_API_KEY` are configured and the selected `payment_methods.auto_verify` is `true` (the DB flag alone), `_verify_payment_automatically()` in `app/services/payments.py` talks to SendHook. All HTTP lives in `app/services/sendhook.py`; `payments.py` never builds a request itself.

Since 2026-09-04 the primary path is SendHook's recommended **pedidos + webhook** flow, not blind polling:

1. `POST {SENDHOOK_API_URL}/pedidos` pre-registers the payment we expect, with **our `payments.id` as `referencia_externa`** — that doubles as SendHook's idempotency key, so a retried registration returns the existing pedido instead of creating a second one.
2. If the SMS/notification had already arrived, the response comes back `"estado": "conciliado"` with the `pago` object and the payment is approved inline (the caller gets the approved row back, same as before).
3. Otherwise it stays `"pendiente"` and SendHook `POST`s `pago.conciliado` to our webhook when the money lands. The registration request does not block on anything.

**Webhook receiver**: `POST /api/webhooks/sendhook` (`app/api/sendhook.py`). Public by necessity, so the auth *is* the HMAC signature — nothing is touched before `sendhook.verificar_firma_webhook()` passes. It checks `X-SendHook-Timestamp` against a ±300s window (anti-replay), then compares `X-SendHook-Signature` to `v1=` + HMAC-SHA256 of `"{timestamp}.{raw_body}"` keyed with `SENDHOOK_WEBHOOK_SECRET`, using `hmac.compare_digest`. **The signature is computed over the raw body** (`await request.body()`), never over a re-serialized JSON dict — key order would change and it would never match. Duplicate deliveries are dropped by `X-SendHook-Event-Id` against the `sendhook_webhook_events` table. Without `SENDHOOK_WEBHOOK_SECRET` set, the endpoint answers `503` (so SendHook retries) rather than `401` (which it would count as a permanent failure). The URL has to be registered by whoever administers SendHook; they hand back the `whsec_...` secret.

**Backstop retries**: `_retry_verification_in_background()` still runs on a daemon thread at 20s/30s/30s/300s, because the webhook depends on that registration existing and can be lost. When a pedido was registered it polls the cheap `GET /pedidos/{id}`; otherwise it falls back to `POST /pagos/verificar` and reads the `motivo` field — `consumido` stops the retries immediately (that payment already approved something else, retrying can never change it), `fuera_de_ventana` widens `ventana_minutos` to the 1440-minute cap, `no_encontrado` keeps waiting. Every path funnels into `approve_from_sendhook()`, which re-checks the payment is still `pending` before approving, so webhook and retry racing each other cannot double-approve.

**Which identifier goes on the wire depends on the destination bank** (`sendhook.build_identifiers()`), and getting this wrong guarantees a `no_encontrado`:

| Destination bank | Sent | Why |
|---|---|---|
| `bdv`, `bnc` | `referencia` only | Filters combine with AND, and these banks store the phone masked (`0424***1486`), so adding our unmasked `contraparte` can only break an otherwise valid match. |
| `bfc` | `contraparte` = payer phone | No reference in the SMS. |
| `binance` | `contraparte` = payer **name** | P2P has neither a reference nor a real phone. `register_with_payment()` passes the registration `name`; `renew_subscription()` reads it from `profiles`. |

Reference matching is on the **last 6 digits**, so leading zeros and bank-code prefixes are harmless (`006687065139`, `6687065139` and `065139` all match) — never ask the user to normalize it. Amounts are serialized by hand (`_build_body`) so `25.50` stays `25.50`; the default JSON encoder would emit `25.5`.

> ⚠️ **`banco` is the destination bank, not the sender's bank.** SendHook's phone is logged into *El Club de Nice's own* receiving account — `banco` must identify which of *our* accounts got the money, never `payments.origin_bank` (that column is the payer's sending bank, entered by the user, kept only for display/manual review). The destination bank is resolved by `_get_destination_bank_slug()`: it reads the value of whichever `payment_method_fields` row has `field_label == "Banco"` for that payment method (the same admin-configured display field users see at checkout, e.g. value `"BNC"` or `"Banco de Venezuela"`) and normalizes free text to SendHook's slug (`bdv`/`bnc`/`bfc`/`binance`) via `sendhook.normalize_bank_label()`. To change which bank auto-verify targets (e.g. switch the business's receiving account from BNC to BDV), just edit that field's value in the payment method's config (admin panel `PUT /api/admin/payment-methods/{id}/values`, or directly in `payment_method_values`) — no code change or redeploy needed. If that value doesn't resolve to a bank SendHook supports (e.g. Banesco), verification is skipped silently and the payment stays `"pending"`.

Anything that never verifies stays `"pending"` for manual review. `reject_payment()` calls `POST /pedidos/{id}/cancelar` first, so a discarded payment doesn't leave a live pedido that could consume a real payment later (a already-conciliated pedido answers 409, which is correct and ignored). On approval, SendHook's own `pago_id` is stored in `payments.sendhook_payment_id` — with Binance that column plus `fecha_operacion` is the only thing distinguishing two identical payments from the same client. No receipt/cédula/fecha is ever sent to SendHook. Phone numbers are normalized to local `0XXXXXXXXXX` by `sendhook.normalize_phone()`, which is defensive against double-prefixed phone bugs like `+5804243771486`. `GET /api/payments/diagnostic-ip` (public) reports the backend's outbound IP — leftover from the previous verification provider, likely no longer needed for SendHook whitelisting.

**Manual re-check (`POST /api/payments/recheck`)**: authenticated, no body. The "Verificar estado" button on the waiting screens (`AccountStatus.tsx`, `RenewalGateway.tsx`) calls it before re-reading `/api/auth/me`, so someone who just paid does not have to wait for the next background retry. `recheck_pending_payment()` finds the user's newest `pending` payment, asks `GET /pedidos/{id}` first (cheap, consumes nothing) and falls back to `POST /pagos/verificar` with the full 1440-minute window, then approves through the same `approve_from_sendhook()` as every other path. It returns `{verificado, estado, message, payment}` where `estado` is one of `aprobado` / `esperando` / `sin_pendiente` / `no_automatico` / `requiere_revision`; `message` is user-facing Spanish already tailored to the `motivo` SendHook gave back, so the frontend renders it verbatim.

Rate limited to **4 calls per user per minute** via `check_user_rate_limit()` in `app/core/rate_limit.py`. That helper is deliberately different from the existing `rate_limiter` dependency in two ways: it keys on the authenticated user id rather than the IP (a mobile NAT puts many members behind one address, and one member changes address moving between wifi and data), and it falls back to an in-process counter when Redis is unavailable instead of degrading to no limit at all — every call costs a real request to SendHook. On block it raises `429` whose detail already names the seconds left; the frontend shows that message as a notice rather than an error.

**DB objects this needs**: `el-club-de-nice/supabase/sendhook-pedidos-webhook.sql` creates `sendhook_webhook_events` (webhook idempotency) and adds `payments.sendhook_payment_id`. Both are handled defensively in code — a missing table or column degrades to a log line rather than a failure — but until the SQL runs, a redelivered webhook is not deduplicated by event id (only by the `pending` re-check).
...
| `send_raffle_winner(to, name, raffle_title)` | `raffles.draw_raffle()` success, per winner | fire-and-forget (thread) |
...
> ⚠️ `analytics_daily_snapshots` does **not** follow these rules. Its rows are written by the Postgres function `generate_daily_snapshot()` (called by the `daily-analytics-snapshot` pg_cron job and by `POST /api/admin/analytics/snapshot`), which lives in Supabase and counts on its own criteria — `invited_members` there also compares role case-sensitively, so capitalized `"Invitado"` rows were missed. The history chart in the admin panel reads those rows, so it's on older criteria than the live figures. A corrected version is checked in at `sql/generate_daily_snapshot.sql`, built as a minimal delta from the function's real definition (dumped with `pg_get_functiondef`): the only changes are the seven member counters, which now carry `FILTER (WHERE lower(trim(role)) = 'miembro')`, plus `invited_members` switching to the same case-insensitive comparison. Everything else — `RETURNS void`, the qualified names, the `ON CONFLICT`, the five payment subqueries — is byte-identical on purpose. **Keep `RETURNS void`**: `CREATE OR REPLACE` cannot change a function's return type, so any other signature fails and would force a `DROP FUNCTION` that momentarily breaks the pg_cron job. Apply it by hand in the Supabase SQL editor; rows written before you do keep the old criteria.
...
- **Winners get an automatic congratulations email**: `draw_raffle()` (manual admin trigger or the `draw-scheduled-raffles` cron) calls `_notify_winners()` after setting `drawn_at`, which sends `email.send_raffle_winner(to, name, raffle_title)` (fire-and-forget, same pattern as `send_welcome`/`send_payment_approved`) to every winner resolved via `_get_email_map()`. A missing email just logs a warning and skips that winner — never blocks the draw.
